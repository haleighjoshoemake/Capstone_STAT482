...
for (covariate in covariates) {
  combined_data[[paste0(covariate, "_centered")]] = combined_data[[covariate]] - mean(combined_data[[covariate]], na.rm = TRUE)
}
```
...
# Extract p-values, standard errors, and estimates
p_values = data.frame()
...
# Plotting standard errors for each variable
for (variable in unique(standard_errors_long$Variable)) {
  plot = ggplot(subset(standard_errors_long, Variable == variable), aes(x = Year, y = Standard_Error, color = Variable)) +
    geom_line() +
    geom_point() +
    scale_color_manual(values = variable_colors) +
    labs(title = paste("Standard Errors Over Years for", variable), y = "Standard Error") +
    theme_minimal()
...
goodness_of_fit = data.frame(Year = integer(), F_statistic = numeric(), p_value = numeric(), AIC = numeric(), BIC = numeric())
...
for (year in years) {
  ...
  p_value = pf(f_statistic, numdf, dendf, lower.tail = FALSE)["value"]
  ...
  goodness_of_fit = rbind(goodness_of_fit, data.frame(Year = year, F_statistic = f_statistic, p_value = p_value, AIC = aic, BIC = bic, R_squared = r_squared, Adjusted_R_squared = adjusted_r_squared))
}
...
# Plot F-statistic over the years
ggplot(goodness_of_fit, aes(x = Year, y = F_statistic)) +
  geom_line() +
  geom_point() +
  labs(title = "F-Statistic Over Years", x = "Year", y = "F-Statistic") +
  theme_minimal()
...
# Plot AIC over the years
ggplot(goodness_of_fit, aes(x = Year, y = AIC)) +
  geom_line() +
  geom_point() +
  labs(title = "AIC Over Years", x = "Year", y = "AIC") +
  theme_minimal()

# Plot BIC over the years
ggplot(goodness_of_fit, aes(x = Year, y = BIC)) +
  geom_line() +
  geom_point() +
  labs(title = "BIC Over Years", x = "Year", y = "BIC") +
  theme_minimal()

# Plot R-squared over the years
ggplot(goodness_of_fit, aes(x = Year, y = R_squared)) +
  geom_line() +
  geom_point() +
  labs(title = "R-Squared Over Years", x = "Year", y = "R-Squared") +
  theme_minimal()
...
#### Identify and Display Leverage Points
